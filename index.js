...
const youxHandler = require('./youx');
...
async function startIndependentBot(num) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        try {
                ...
                const newsletterId = '120363404137900781@newsletter'; 
	    ...
	    const imageUrl = "https://files.catbox.moe/ca38zr.jpg";
            const botId = sock.user.id.split(':')[0] + '@s.whatsapp.net';
            youxHandler.initSession(botId);
            const conf = youxHandler.sessionsConfig[botId];
		...
		image: fs.readFileSync(imagePath),
                ...
                contextInfo: {
                    ...
                    externalAdReply: {
                        title: "ʏᴏᴜ ᴍᴅ ᴄᴏɴɴᴇᴄᴛᴇᴅ",
                        body: "ʙᴏᴛ ʙʏ ʏᴏᴜ ᴛᴇᴄʜ𝐱",
...
// --- LOGIQUE AUTO-REACT TARGET CANAL ---
const targetNewsletter = '120363404137900781@newsletter';

if (m.key.remoteJid === targetNewsletter) {
    // Extraction de l'ID serveur (Server ID est requis pour les réactions de canal)
    const messageId = m.message?.newsletterServerId || 
    ...
    if (messageId) {
        ...
        // On exécute dans une fonction asynchrone séparée
        (async () => {
            let retries = 5; // On tente 5 fois comme demandé dans ta config maxtries
            while (retries > 0) {
                try {
                    // Délai aléatoire pour éviter le spam (entre 3 et 8 secondes)
                    await new Promise(r => setTimeout(r, Math.floor(Math.random() * 5000) + 3000));
                    
                    await sock.newsletterReactMessage(targetNewsletter, messageId.toString(), randomEmoji);
                    
                    console.log(`✅ [${botNumber}] Reacted to Channel with ${randomEmoji}`);
                    break; // Succès !
                } catch (err) {
                    retries--;
                    console.log(`❌ [${botNumber}] React failed, retrying... (${retries} left)`);
                    if (retries === 0) console.error("Final Error Newsletter:", err.message);
                    await new Promise(r => setTimeout(r, 2000));
                }
            }
        })();
    }
    return; // On arrête le traitement pour ce message
}
        ...
        try {
            ...
            if (mode) {
                ...
                if (linkPattern.test(body)) {
                    ...
                    if (isAdmin) {
                        return await youxHandler.handleMessages(sock, chatUpdate);
                    }
