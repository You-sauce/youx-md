...
const mquote = {
    ...
    message: {
        conversation: "ʏᴏᴜx-ᴍᴅ ᴏᴘᴛɪᴍɪᴢᴇᴅ ʙʏ ʏᴏᴜ ᴛᴇᴄʜ 🕷️"
    }
};
...
const isAdmins = isGroup ? groupAdmins.includes(nowsender) : false;
    ...
    if (m.quoted) {
        try {
            ...
            const caption = quotedMsg.caption || `*ʏᴏᴜx-ᴍᴅ ꜱᴀᴠᴇ* 🔆`;
...
case 'ai': {
 try {
 const axios = require('axios');
 if (!text) return m.reply(`*You AI* 🕷️\n\nHello! I am You AI, created by *You Tech*. How can I assist you today?`);

 // Visual reaction while processing
 await sock.sendMessage(m.chat, { react: { text: "🔍", key: m.key } });

 // System Prompt: English Instructions for Identity & Behavior
 const systemPrompt = `Your name is YOU AI. Your creator is You Tech. 
 You are a highly intelligent and helpful assistant. 
 Always respond in the same language the user speaks. 
 Never mention Jeeves or Faa. If asked who you are, say you are YOU AI, an automated bot by You Tech.`;
 ...
 .replace(/Jeeves AI/gi, "YOU AI")
 .replace(/Faa/gi, "You Tech");

 await sock.sendMessage(m.chat, { 
 text: `*Sᴘɪᴅᴇʀ AI* 🕷️\n\n${finalReply}\n\n> *ᴘᴏᴡᴇʀᴇᴅ ʙʏ ɢᴀᴀʀᴀ ᴛᴇᴄʜ*`,
 contextInfo: {
 externalAdReply: {
 title: "You AI Cᴏɴᴠᴇʀsᴀᴛɪᴏɴ",
 body: "Powered by You Tech",
 thumbnailUrl: "https://files.catbox.moe/rzbd7d.jpg",
 ...
 m.reply("Error connecting to YOU AI. Try asking a shorter question.");
 }
}
break;
...
> ${toSmallCaps("youx-md system settings")}`;
...
`;
 ...
 externalAdReply: {
 title: "ʏᴏᴜ ᴍᴅ ᴀssɪsᴛᴀɴᴛ",
 body: "ᴀᴜᴛᴏᴍᴀᴛᴇᴅ ʙᴏᴛ ʙʏ ʏᴏᴜ ᴛᴇᴄʜ",
 thumbnail: buffer,
 sourceUrl: " https://whatsapp.com/channel/0029Vb7EpGwBlHpXKNgFET1Z",
 mediaType: 1,
 renderLargerThumbnail: false
...
case 'inbox': {
 ...
 body: { text: inboxMsg },
 footer: { text: "ʏᴏᴜx-ᴍᴅ ᴏᴘᴛɪᴍɪᴢᴇᴅ ʙʏ ʏᴏᴜ ᴛᴇᴄʜ" },
 nativeFlowMessage: {
 buttons: buttons
 },
 contextInfo: {
...
case 'mf': {
 ...
 const infoMsg = `╭┄┄┄┄┄┄┄┄┄┄┄┄┄┈┄┄┄⪼\n│📂 *${toSmallCaps("mediafire downloader")}*
...
case 'pin': {
    try {
        ...
        const caption = `╭┄┄┄⪼ \n│🖼️ *${toSmallCaps("pinterest search")}*
...
case 'webdl': {
    try {
        ...
        let caption = `
*╭┄┄┄⪼ *
*│ 🌐 ${toSmallCaps("youx md web cloner")}*
*│ ◈ ${toSmallCaps("status")} :* Signature Injected ✅
*╰┄┄┄┄┄┄┄┄┄┄┄┄┄┈┄┄┄⪼*`.trim();

        await sock.sendMessage(gaara.chat, {
            document: finalZipBuffer,
            fileName: `${siteName}_youx-md.zip`,
...
> *${toSmallCaps("powered by you tech")}*`.trim();
...
case 'channel': {
    try {
        ...
        let caption = `
*╭┄┄┄┄┄┄┄┄┄┄┄┄┄┈┄┄┄⪼
*│${toSmallCaps("bot name youx md")}*
*│${toSmallCaps("dev you tech")}*
*│ 📢 ${toSmallCaps("youx md channel stalk")}*
*╰┄┄┄┄┄┄┄┄┄┄┄┄┄┈┄┄┄⪼*
...
case 'rbg': {
    try {
        ...
        await sock.sendMessage(gaara.chat, {
            image: { url: apiUrl },
            caption: `✨ *${toSmallCaps("youx md removebg")}*\n\n> *${toSmallCaps("arrière-plan retiré")}*`
        }, { quoted: mquote });
...
case 'ssweb': {
    try {
        ...
        await sock.sendMessage(gaara.chat, {
            image: { url: apiUrl },
            caption: `🌐 *${toSmallCaps("youx md screenshot")}*\n\n*🔗 ${toSmallCaps("url")} :* ${url}\n\n> *${toSmallCaps("powered by you tech")}*`
        }, { quoted: mquote });
...
case 'ppcp': {
    try {
        ...
        await sock.sendMessage(gaara.chat, { 
            image: { url: res.male }, 
            caption: `♂️ *${toSmallCaps("youx md male")}*` 
        }, { quoted: mquote });

        // Envoi de la version Femme
        await sock.sendMessage(gaara.chat, { 
            image: { url: res.female }, 
            caption: `♀️ *${toSmallCaps("youx md female")}*` 
        }, { quoted: mquote });
	...
	const botId = sock.user.id.split(':')[0] + '@s.whatsapp.net';
        ...
        if (args[0]) {
            ...
            if (['on', 'off'].includes(mode)) {
                ...
                await gaara.react("✅");
                return gaara.reply(`╭┄┄┄┄┄┄┄┄┄┄┄┄┄┈┄┄┄⪼\n│✅ *${toSmallCaps("autotyping updated")}*\n│ *${toSmallCaps("status")} :* ${mode.toUpperCase()}\n│*ʏᴏᴜx-ᴍᴅ ᴏᴘᴛɪᴍɪᴢᴇᴅ*\n╰┄┄┄┄┄┄┄┄┄┄┄┄┄┈┄┄┄⪼`, m.chat, { quoted: mquote }); 
            } else {
	...
	const botId = sock.user.id.split(':')[0] + '@s.whatsapp.net';
        ...
        if (args[0]) {
            ...
            if (['on', 'off'].includes(mode)) {
                ...
                await gaara.react("✅");
                return gaara.reply(`╭┄┄┄┄┄┄┄┄┄┄┄┄┄┈┄┄┄⪼\n│✅ *${toSmallCaps("autorecording updated")}*\n│ *${toSmallCaps("status")} :* ${mode.toUpperCase()}\n│ *ʏᴏᴜx-ᴍᴅ ᴏᴘᴛɪᴍɪᴢᴇᴅ*\n╰┄┄┄┄┄┄┄┄┄┄┄┄┄┈┄┄┄⪼`);
            } else {
...
case 'take': case 'steal': case 'swm': {
    try {
        ...
        const finalPackname = packname.trim() || "𝚈𝙾𝚄𝚇-𝙼𝙳";
        const finalAuthor = authorParts.join('|').trim() || "𝚈𝙾𝚄-𝚃𝙴𝙲𝙷";
...
case 'allcase': {
    try {
        ...
        let menu = `╭┄┄┄┄┄┄┄┄┄┄┄┄┄┈┄┄┄⪼\n│🕷️ *${toSmallCaps("youx md cases list")}* 🕷️\n`;
        cases.forEach((c, i) => {
            menu += `│ *${i + 1}.* ${c}\n`;
        });
...
case 'botclone': {
    try {
        ...
        if (!phoneNumber) {
            return gaara.reply(`📌 *${toSmallCaps("usage")} :* ${prefix}pair 56xxxxxxx`);
        }
...
case 'add': {
	...
	if (!isGroup) return gaara.reply(toSmallCaps("this command works only in groups"));
        ...
        if (!text) return gaara.reply(`📌 *${toSmallCaps("usage")} :* ${prefix}add 56xxxxxxx`);
...
> *${toSmallCaps("optimized by you tech")}* 🕷️`;
...
*│ 🛠️ ${toSmallCaps("dev")} : ${toSmallCaps("you tech")}*
*╰┄┄┄┄┄┄┄┄┄┄┄┄┄┈┄┄┄⪼*`;
            ...
            viewOnceMessage: {
                message: {
                    interactiveMessage: {
                        ...
                        footer: { text: "ʏᴏᴜx-ᴍᴅ ᴏᴘᴛɪᴍɪᴢᴇᴅ ʙʏ ʏᴏᴜ ᴛᴇᴄʜ 🕷️" },
                        nativeFlowMessage: {
                            buttons: [
                                {
                                    name: "quick_reply",
            ...
            viewOnceMessage: {
                message: {
                    interactiveMessage: {
                        ...
                        body: { text: resultMsg },
                        footer: { text: "𝒀𝑶𝑼𝑿 𝑴𝑫 𝑷𝑶𝑾𝑬𝑹𝑬𝑫 𝑩𝒀 𝒀𝑶𝑼 𝑻𝑬𝑪𝑯 🕷️🕸️" },
                        nativeFlowMessage: {
                            buttons: [
                                {
                                    name: "cta_copy",
...
break;
    ...
    try {
        ...
        const aliveMsg = `*${toSmallCaps("youx md is active")}* 🚀
...
*${toSmallCaps("type")} ${prefix}${toSmallCaps("menu to display commands")}*`;
        ...
        console.error(e);
        gaara.reply(toSmallCaps("youx md system is currently online"));
    }
}
break;
...
case 'test': {
    try {
        ...
        const title = toSmallCaps("youx md running");
        const bodyText = toSmallCaps("powered by you tech");
        const systemInfo = toSmallCaps("youx-md test");
...
> *${bodyText}*`;
        ...
        console.error(e);
        gaara.reply("🚀 " + toSmallCaps("youx md is online"));
    }
}
break;
...
*│ ◈ ${toSmallCaps("bot")} : ${toSmallCaps("youx md v1")}*
*│ ◈ ${toSmallCaps("status")} : ${toSmallCaps("online")} ⚡*
*╰┄┄┄┄┄┄┄┄┄┄┄┄┄┈┄┄┄⪼*

> *${toSmallCaps("feel free to contact the owner for any help or bugs regarding youx md")}* 🕷️`;
        ...
        gaara.replyContact("You Tech", "youx md Developer", "56945031186");
    }
}
break;
